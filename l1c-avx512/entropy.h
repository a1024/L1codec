...
static void ansval_check(const void *data, int esize, int count)
{
	...
	if(esize!=loheader->esize||count!=loheader->count||memcmp(data, data0, count*esize))
	{
		...
		printf("\n");
#if 1
		for(int k=0;k<loheader->count;++k)
		{
			for(int k2=loheader->esize-1;k2>=0;--k2)
				printf("%02X", data0[k*loheader->esize+k2]);
			printf(" ");
			for(int k2=loheader->esize-1;k2>=0;--k2)
				printf("%02X", ((const unsigned char*)data)[k*loheader->esize+k2]);
			printf(" ");
			for(int k2=loheader->esize-1;k2>=0;--k2)
				printf("%02X", data0[k*loheader->esize+k2]^((const unsigned char*)data)[k*loheader->esize+k2]);
			printf("\n");
		}
#endif
